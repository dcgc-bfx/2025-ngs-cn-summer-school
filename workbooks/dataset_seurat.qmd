...
Currently, there are no reductions in the Seurat object. If they were present, you could access them using:
...
Currently, there are no computed neighbors or graphs. If available, you could access them with:
...
In spatial transcriptomics (and any sequencing-based assay), not all bins (or cells) are equally informative. Quality Control (QC) is used to identify and remove low-quality bins from the dataset before further analysis. 
