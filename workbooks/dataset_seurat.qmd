...
To run RCTD, we first install the spacexr package from GitHub which implements RCTD. When running RCTD, we follow the instructions from the RCTD vignette.
...
ref = Reference(counts, cluster, nUMI, n_max_cells=1000) # set low for demo
...
counts = GetAssayData(sc, assay="sketch", layer="counts")
bins_sketch = Cells(sc[["sketch"]])
coords = GetTissueCoordinates(sc)[bins_sketch, 1:2]
query = SpatialRNA(coords, counts, colSums(counts))
```

```{r}
RCTD = create.RCTD(query, ref, max_cores = 25, fc_cutoff = 1, fc_cutoff_reg = 1) # set high for demo
```
