...
# Parallelisation seems to be slower - remove?
#plan("multisession", workers = 4)
#options(future.globals.maxSize=+Inf)
plan("sequential")

# Latest (May2025) Seurat and SeuratObject seem to have a number of bugs. Downgrade for now here but also in the image?
#library(devtools)
#install_version("Seurat", version = "5.2.1")
#install_version("SeuratObject", version = "5.0.2")
...
#grid.newpage()
grid.draw(GetImage(sc[["slice1.008um"]], method="grob"))
```
...
TODODODODODODO
::: {.callout-tip}
You might see strange artefacts in the image. This can happen if you plot the image using relatively low resolution (150dpi) and then open it on a high-resolution display. In this case, you need to increase the resolution using the quarto option `dpi`. Alternatively decrease the figure size.
:::
...
Here, we determine the top 1000 HGVs: 

```{r}
sc = FindVariableFeatures(sc, selection.method="vst", nfeatures=1000)
#FOR NOW we take 1000 for memory issues
```
...
The Seurat object now contains a full PCA (`pca.008um`), a full UMAP (`umap.008um`) and a full clustering (`seurat_clusters.008um`). We plot the clustering of the full dataset on the UMAP as well as in spatial context:
...
# Store the BANKSY layer data more compressed
data = as(GetAssayData(sc[["BANKSY.008um"]], layer="data"), "dgCMatrix")
sc[["BANKSY.008um"]] = SetAssayData(sc[["BANKSY.008um"]], layer="data", new.data=data)
rm(data)
gc(verbose=FALSE)
...
```{r}
sc = RunPCA(sc, reduction.name="banksy_pca.008um", features=rownames(sc[["BANKSY.008um"]]), npcs=15)
sc = FindNeighbors(sc, reduction="banksy_pca.008um", dims=1:15)
sc = FindClusters(sc, cluster.name="banksy_clusters.008um", resolution=0.5, algorithm=4)
...
The same comparison as spatial plot:

```{r}
p1 = SpatialDimPlot(sc, group.by="seurat_clusters.008um") + NoLegend() + ggtitle("Seurat clustering")
p2 = SpatialDimPlot(sc, group.by="banksy_clusters.008um") + NoLegend() + ggtitle("BANKSY clustering")
p1 | p2
```

To decide which is one is more

## Find spatial domains with BANKSY

We set `lambda` to 0.8 and the number of neighbors (`k_geom`) to 50. Note that we fist delete the BANKSY assay from before. This is not strictly necessary but it will save us some memory.
