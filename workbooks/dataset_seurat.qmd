...
```{r}
#| message: true
GetAssayData(sc, assay = "RNA.008um", layer = "counts") %>% tail(5)
```
...
This creates a new layer in the assay object called `data` that contains the normalized counts:

```{r}
sc[["RNA.008um"]]
```

We can inspect the normalized counts with the `GetAssayData` function:

```{r}
GetAssayData(sc, layer = "data") %>% tail(3)
```
