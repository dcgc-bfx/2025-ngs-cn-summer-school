...
Filter out genes expressed in ≤10 bins:
...
p = SpatialDimPlot(sc, cells.highlight=cluster_bins, facet.highlight=TRUE, cols.highlight=c("#FFFF00", "grey50")) + NoLegend()
p
```
...
**⚠️ Important**: BANKSY fills the `scale.data` layer. Therefore do not call `ScaleData` on the BANKSY assay as this negates the effects of lamda. 
...
We now load the [Allen Brain single-cell reference dataset](https://www.dropbox.com/scl/fi/r1mixf4eof2cot891n215/allen_scRNAseq_ref.Rds?rlkey=ynr6s6wu1efqsjsu3h40vitt7&dl=0), which has been saved as a Seurat object:
...
Now we could run RCTD. However, it would take too long for this tutorial. Therefore, we have already run this analysis for you and saved the results in the file `datasets/RCTD.Rds`. All you need to do today is to load it: 

We now run the RCTD algorithm. This may take some time depending on the number of bins and used cores.
...
The RCTD object contains the results of the deconvolution analysis. The results are stored in the `results_df` slot. Here are explanations of the most important columns:

- `spot_class`: indicates whether this bin is a "singlet" (one cell), "doublet_certain" (two cells), "doublet_uncertain" (two cells but uncertain), "reject" (cannot say)
- `first_type`: gives the prediction for the first cell type
- `second_type`: only if the bin is a doublet (!), gives the prediction for the second cell type

For all other columns, we refer to the RCTD documentation. We will add the three columns to the bin metadata of the cortex object:
...
Now we need to project the RCTD results of the cortex sketch dataset back to the cortex full dataset. This is similar to the projection of the clustering and UMAP results we did before. 
...
Ultimately, we would like to include the cell type predictions in our original brain dataset. We add them to the bin metadata:
...
First, we can plot different cell types. For example, here we show three different layers of excitatory neurons (L2-3 IT CTX, L4-5 IT CTX, L5 IT CTX):
...
p = SpatialDimPlot(sc, cells.highlight=layered_excitatory_bins, facet.highlight=TRUE, cols.highlight=c("#FFFF00", "grey50")) + NoLegend()
p
```

## Annotate seurat_clusters, BANKSY clusters, BANKSY domains

With this cell type information, we can annotate the clusters of our basic workflow and of BANKSY. It also helps us to identify which clusters need to be merged and which to be split. **However**: Remember that in this tutorial we did cell type predictions only for defined subset. Hence results might only be meaningfull for a subset of clusters.

Here are the predicted cell types for the clusters of our basic workflow:

```{r}
cell_type_seurat_cluster_counts = sc[[]] %>% 
         dplyr::count(first_cell_type.008um, seurat_clusters.008um)

ggplot(cell_type_seurat_cluster_counts,
       aes(x=seurat_clusters.008um, y=n, fill=first_cell_type.008um)) +
  geom_bar(stat = "identity", position = "fill")
```

```{r}
cell_type_banksy_cluster_counts = sc[[]] %>% 
         dplyr::filter(first_cell_type.008um != "Unknown") %>%
         dplyr::count(first_cell_type.008um, banksy_clusters.008um)

ggplot(cell_type_banksy_cluster_counts,
       aes(x=banksy_clusters.008um, y=n, fill=first_cell_type.008um)) +
  geom_bar(stat = "identity", position = "fill")
```


With this cell type information, we can annotate the spatial domains we identified with BANKSY. Here, we will see whether we can identify all domains that are associated with excitatory neuron layers annotated in the Allen Brain Atlas:

```{r}
layered_excitatory_neurons = c("L2-3 IT CTX", "L4-5 IT CTX", "L5 IT CTX", "L5 PT CTX", "L5-6 NP CTX", "L6 CT CTX", "L6 IT CTX", "L6b CTX")

# First we count the number of bins predicted to contain excitatory neurons per BANKSY domain:
cell_type_banksy_domain_counts = sc[[]] %>%
  dplyr::filter(first_cell_type.008um %in% layered_excitatory_neurons) %>%
  dplyr::count(first_cell_type.008um, banksy_domains.008um)

# Then we use ggplot
p = ggplot(cell_type_banksy_domain_counts,
       aes(x=banksy_domains.008um, y=n, fill=first_cell_type.008um)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(label = ifelse(n>50, as.character(first_cell_type.008um), "")), 
                position = position_stack(vjust = 0.5), size = 3) +
  ylab("# of bins") +
  ggtitle(paste0("Distribution of Cell Types across BANKSY domains")) +
  theme_minimal()
print(p)
```
