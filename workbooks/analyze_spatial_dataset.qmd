...
A **dot plot** allows for the visualization of gene expression across multiple clusters for several genes. We have demonstrated the dot plot already when looking for marker genes - so we do not show it here again.
...
We now project the RCTD results from the cortex sketch dataset to the full cortex dataset. This is similar to the projection of the clustering and UMAP results we did before:
...
We can now visualize cell type predictions. For example, here we highlight three predicted excitatory neuron layers:
...
Using the predicted cell types, we can now annotate our clusters and domains. Keep in mind that predictions were only generated for a subset of the dataset, so results may not apply to all clusters.
...
We now evaluate the spatial domains in terms of excitatory neuron layer annotations from the Allen Brain Atlas.
...
# Then we use ggplot
ggplot(cell_type_banksy_domain_counts,
...
Previously, you learned already about the Loupe Browser as handy tool for browsing through your data. In this section, we will export the results of our analysis as CSV files so that they can be imported into Loupe:
...
Information about importing results into Loupe can be found in the [Loupe tutorial](https://www.10xgenomics.com/support/software/loupe-browser/latest/tutorials/introduction/lb-sc-interface-and-navigation).
 ...
 sc[["im"]] = Crop(im, x = c(10000, 11000), y = c(8000, 9000), coords = "tissue")
