...
# Only analyze cluster for which at least 100 bins have cell type predictions (not unknown)
cluster_of_interest = sc[[]] %>%
  dplyr::group_by(seurat_clusters.008um) %>%
  dplyr::summarise(bins_annotated = sum(first_cell_type.008um != "Unknown") / length(first_cell_type.008um)) %>%
  dplyr::filter(frac_annotated >= 0.75) %>%
  dplyr::pull(seurat_clusters.008um)

# Count cell types per cluster
cell_type_seurat_cluster_counts = sc[[]] %>% 
         dplyr::filter(seurat_clusters.008um %in% cluster_of_interest) %>%
...
We see that many clusters contain multiple prediced cell types and that clustering still needs to be improved. However it is also clear that many bins could contain multiple cell types which makes clustering more difficult.

Annotation may work better for larger structures such as the `banksy_domains`:
