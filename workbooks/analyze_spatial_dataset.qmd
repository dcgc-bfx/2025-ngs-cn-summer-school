...
To make the analysis computationally manageable, we sketch the dataset down to 50,000 bins for this tutorial, then perform standard scaling and PCA:
...
Now we could run RCTD. MENTION DOUBLET MODE. However, it would take too long for this tutorial. Therefore, we have already run this analysis for you and saved the results in the file `datasets/RCTD.Rds`. All you need to do today is to load it. 
...
RCTD@results$results_df %>% head()
```

The RCTD object contains the results of the deconvolution analysis. The results are stored in the `results_df` slot, which contains the estimated cell type proportions for each bin in the query dataset. 
...
cortex = ProjectData(cortex,
                 sketched.assay="sketch", assay="RNA.008um", 
                 sketched.reduction="pca.sketch", full.reduction="pca.008um", 
                 refdata=list(first_type.008um="first_type"),
                 dims=1:30)

sc[[]][, "first_type.008um"] <- "Unknown"
sc$first_type.008um[Cells(cortex)] <- cortex$first_type.008um[Cells(cortex)]
```
